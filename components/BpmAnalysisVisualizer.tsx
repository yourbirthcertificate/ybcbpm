...
export const BpmAnalysisVisualizer: React.FC<BpmAnalysisVisualizerProps> = ({ candidates, detectedBpm }) => {
  ...
  return (
    <div className="mt-8 text-left animate-fade-in">
        ...
        <div className="glass-card p-6 rounded-3xl flex justify-around items-end h-52 border border-white/10">
            {topCandidates.map((candidate) => {
                ...
                return (
                    <div key={candidate.tempo} className="flex flex-col items-center h-full justify-end" title={`BPM: ${candidateBpmFormatted} | Score: ${candidate.count}`}>
                        <div
                            className={`w-8 md:w-10 rounded-t-2xl transition-all duration-500 ease-out hover:opacity-90 ${isDetected ? 'bg-gradient-to-t from-sky-500 via-blue-500 to-purple-500 shadow-[0_20px_35px_rgba(56,189,248,0.35)]' : 'bg-slate-600/70'}`}
